...
class ServiceTests(unittest.TestCase):
    def test_systemd_dependency_graph(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            for name in names:
                (units/name).write_text(((ASSETS/name) if (ASSETS/name).exists() else (DEVICE/name)).read_text())
            ...
            for name in ['usr/local/bin/'+n for n in ['widelapse-grow-data','widelapse-init-directories','widelapse-init-identity','widelapse-init-ota','widelapse-rauc-confirm']]+['bin/true']:
