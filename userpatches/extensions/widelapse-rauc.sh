...
function user_config__widelapse_rauc() {
    ...
    declare -g EXTRA_IMAGE_SUFFIX="${EXTRA_IMAGE_SUFFIX:-}-rauc-ab"
    add_packages_to_image rauc u-boot-tools libubootenv-tool cloud-guest-utils curl ca-certificates python3 e2fsprogs util-linux
}
...
settings = {
 ...
 'CMD_FS_GENERIC':'y', 'CMD_BOOTI':'y', 'CMD_SOURCE':'y', 'CMD_TEST':'y',
 'USE_BOOTCOMMAND':'y',
...
function uboot_make_config__widelapse_rauc_validate() {
    ...
    for setting in CONFIG_ENV_IS_IN_MMC=y CONFIG_ENV_REDUNDANT=y \
        ...
        CONFIG_CMD_FS_GENERIC=y CONFIG_CMD_BOOTI=y CONFIG_CMD_SOURCE=y CONFIG_CMD_TEST=y; do
...
function format_partitions__widelapse_rauc() {
    mkfs.ext4 -F -m 0 -L widelapse-data "${LOOP}p4"
